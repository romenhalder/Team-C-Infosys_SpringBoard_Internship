...
const TransactionHistory = () => {
    ...
    const getTypeColor = (type) => {
        switch (type) {
            case 'STOCK_IN': return 'bg-emerald-500/100/15 text-emerald-300';
            case 'STOCK_OUT': return 'bg-rose-500/15 text-rose-300';
            case 'ADJUSTMENT': return 'bg-sky-500/15 text-sky-300';
            case 'RETURN': return 'bg-amber-500/100/15 text-amber-300';
            case 'WASTAGE': return 'bg-orange-500/15 text-orange-300';
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-cyan-400">Transaction Log</h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="card bg-emerald-500/100/10 border-l-4 border-green-500">
                    <p className="text-xs text-emerald-400 font-medium">Total Stock In</p>
                    <p className="text-2xl font-bold text-emerald-300">{summaryStats.totalIn}</p>
                </div>
                <div className="card bg-rose-500/10 border-l-4 border-red-500">
                    <p className="text-xs text-rose-400 font-medium">Total Stock Out</p>
                    <p className="text-2xl font-bold text-rose-300">{summaryStats.totalOut}</p>
                </div>
                <div className="card bg-orange-500/10 border-l-4 border-orange-500">
                    <p className="text-xs text-orange-400 font-medium">Total Wastage</p>
                    <p className="text-2xl font-bold text-orange-300">{summaryStats.totalWaste}</p>
                </div>
                <div className="card bg-sky-500/10 border-l-4 border-blue-500">
                    <p className="text-xs text-sky-400 font-medium">Total Transactions</p>
                    <p className="text-2xl font-bold text-sky-300">{summaryStats.totalTx}</p>
                </div>
            </div>
                ...
                <div className="flex flex-wrap gap-4 items-center">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            className="px-3 py-1.5 text-sm border border-slate-600 rounded-lg focus:outline-none focus:ring-1 focus:ring-cyan-400"
                        >
                    ...
                    <div className="flex-1 min-w-[200px]">
                        <input
                            ...
                            className="w-full px-3 py-1.5 text-sm border border-slate-600 rounded-lg focus:outline-none focus:ring-1 focus:ring-cyan-400"
                        />
                    </div>

                    <button
                        onClick={() => setSortOrder(sortOrder === 'newest' ? 'oldest' : 'newest')}
                        className="px-3 py-1.5 text-xs font-medium rounded-full bg-slate-800 text-slate-400 hover:bg-slate-700 whitespace-nowrap"
                    >
                    ...
                    <table className="min-w-full divide-y divide-slate-700">
                        <thead className="bg-slate-800/50">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Qty Change</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Before → After</th>
                                ...
                                sortedTransactions.map((tx) => (
                                    <tr key={tx.id} className="hover:bg-slate-800/50 transition-colors">
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <div className="text-sm font-medium text-slate-100">{formatDate(tx.transactionDate)}</div>
                                            <div className="text-xs text-slate-500">{formatTime(tx.transactionDate)}</div>
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <div className="text-sm font-medium text-slate-100">{tx.productName}</div>
                                            <div className="text-xs text-slate-500">{tx.productCode || ''}</div>
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap">
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <span className={`text-sm font-bold ${tx.quantity > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
