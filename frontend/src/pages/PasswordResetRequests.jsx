  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-cyan-400">Password Reset Requests</h1>
          <p className="text-slate-400 mt-1">Manage pending password reset requests from employees and managers</p>
        </div>
        <div className="bg-amber-500/100/15 text-amber-300 px-4 py-2 rounded-lg">
          <span className="font-semibold">{requests.length}</span> Pending Requests
        </div>
      </div>

      {message && (
        <div className="p-4 bg-emerald-500/100/15 text-emerald-300 rounded-lg">
      ...
      {error && (
        <div className="p-4 bg-rose-500/15 text-rose-300 rounded-lg">
          {error}
        </div>
      )}
      ...
      {requests.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-slate-400">There are no password reset requests waiting for your approval.</p>
        </div>
      ) : (
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <div className="flex-1 relative">
                      <input
                        ...
                        className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent ${validationErrors[request.id] ? 'border-red-500 bg-rose-500/10' : 'border-slate-600'
                          }`}
      ...
      <div className="card bg-sky-500/10 border-blue-200">
        <h3 className="font-semibold text-sky-300 mb-2">How it works:</h3>
        <ul className="text-sm text-sky-300 space-y-2 list-disc list-inside">
