...
const salesSlice = createSlice({
  ...
  initialState: {
    currentSale: null,
    recentSales: [],
    todaySummary: { totalSales: 0, transactionCount: 0 },
    cart: [],
    loading: false,
    error: null,
    success: false,
  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      const existingItem = state.cart.find((item) => item.medicationId === product.id);
    ...
    updateCartItem: (state, action) => {
      const { medicationId, quantity } = action.payload;
    ...
    removeFromCart: (state, action) => {
      state.cart = state.cart.filter((item) => item.medicationId !== action.payload);
    },
    clearCart: (state) => {
      state.cart = [];
  ...
  extraReducers: (builder) => {
    builder
      .addCase(createSale.pending, (state) => { state.loading = true; state.error = null; state.success = false; })
      ...
      .addCase(createSale.rejected, (state, action) => { state.loading = false; state.error = action.payload; state.success = false; })
      .addCase(fetchRecentSales.pending, (state) => { state.loading = true; })
      .addCase(fetchRecentSales.fulfilled, (state, action) => { state.loading = false; state.recentSales = action.payload; })
      .addCase(fetchRecentSales.rejected, (state, action) => { state.loading = false; state.error = action.payload; })
      .addCase(fetchSaleByOrderNumber.fulfilled, (state, action) => { state.currentSale = action.payload; })
      .addCase(fetchTodaySummary.fulfilled, (state, action) => { state.todaySummary = action.payload; });
  },
});
