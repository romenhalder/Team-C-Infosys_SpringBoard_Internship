...
const StockUpdate = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-3xl font-bold text-cyan-400 mb-6">Update Stock</h1>

      {error && (
        <div className="mb-4 p-4 bg-rose-500/15 text-rose-300 rounded-lg">
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          ...
          {selectedProduct && (
            <div className="mt-2 p-2 bg-sky-500/10 rounded text-xs text-sky-300">
        ...
        {/* Transaction Type */}
        <div>
          <label className="block text-sm font-medium text-slate-300">Transaction Type *</label>
          <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[
              { value: 'STOCK_IN', label: 'Stock In', color: 'bg-emerald-500/100/15 text-emerald-300' },
              { value: 'STOCK_OUT', label: 'Stock Out', color: 'bg-rose-500/15 text-rose-300' },
              { value: 'ADJUSTMENT', label: 'Adjustment', color: 'bg-sky-500/15 text-sky-300' },
              { value: 'RETURN', label: 'Return', color: 'bg-amber-500/100/15 text-amber-300' },
              { value: 'WASTAGE', label: 'Wastage', color: 'bg-orange-500/15 text-orange-300' },
            ].map((type) => (
              <label
                key={type.value}
        ...
        {formData.type === 'STOCK_IN' && selectedProduct && (
          <div className="p-4 bg-sky-500/10 border border-blue-200 rounded-lg space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-sky-300">Price Verification</p>
                <p className="text-xs text-sky-400">Current cost price: <strong>₹{selectedProduct.costPrice || 0}</strong></p>
              </div>
            </div>
            ...
            {showPriceChange && (
              <div>
                <label className="block text-sm font-medium text-sky-300">New Cost Price (for this batch)</label>
                <input
                  type="number"
                  value={newCostPrice}
        ...
        {formData.type === 'STOCK_OUT' && !isRawMaterial && (
          <div>
            <label className="block text-sm font-medium text-slate-300">Unit Selling Price (Optional)</label>
            <input
              type="number"
              name="unitPrice"
        ...
        {formData.type === 'STOCK_OUT' && isRawMaterial && (
          <div className="p-3 bg-amber-500/100/10 border border-yellow-200 rounded-lg">
        ...
        {selectedProduct && formData.quantity && (
          <div className="bg-slate-800/50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-slate-300 mb-2">Transaction Summary</h3>
            <div className="space-y-1 text-sm">
              <p><span className="text-slate-400">Product:</span> {selectedProduct.name}</p>
              <p><span className="text-slate-400">Current Stock:</span> {selectedProduct.currentStock || 0}</p>
              <p>
                <span className="text-slate-400">New Stock:</span>{' '}
                <span className={`font-medium ${['STOCK_IN', 'ADJUSTMENT'].includes(formData.type) ? 'text-emerald-400' : 'text-rose-400'
                  }`}>
              ...
              {showPriceChange && newCostPrice && (
                <p>
                  <span className="text-slate-400">New Cost Price:</span>{' '}
                  <span className="font-medium text-sky-400">₹{newCostPrice}</span> (for {formData.quantity} units)
                </p>
              )}
              {formData.unitPrice && (
                <p>
        ...
        <div className="flex justify-end space-x-4 pt-4 border-t">
          ...
          <button
            ...
            className={`px-6 py-2 rounded-lg text-white font-medium ${formData.type === 'STOCK_IN'
              ? 'bg-emerald-500/100 hover:bg-emerald-500'
