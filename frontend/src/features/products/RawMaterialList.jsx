...
const RawMaterialList = () => {
    ...
    const handleDelete = (id) => {
        if (window.confirm('Are you sure you want to delete this generic drug?')) {
    ...
    const getStockStatus = (product) => {
        if (product.currentStock === 0 || product.isOutOfStock || product.currentStock < 0) {
            return { text: 'Out of Stock', class: 'bg-rose-500/15 text-rose-300' };
        } else if (product.isLowStock || product.currentStock <= product.minStockLevel) {
            return { text: 'Low Stock', class: 'bg-amber-500/100/15 text-amber-300' };
        } else {
            return { text: 'In Stock', class: 'bg-emerald-500/100/15 text-emerald-300' };
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">Generic Drugs / API</h1>
                {canEdit && (
                    <button
            ...
            <div className="text-sm text-slate-400">
                Showing {sortedMaterials.length} generic drugs
            </div>
                    ...
                    <table className="min-w-full divide-y divide-slate-700">
                        <thead className="bg-slate-800/50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">Cost Price</th>
                        ...
                        <tbody className="bg-slate-900 divide-y divide-slate-700">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan="7" className="px-6 py-4 text-center text-slate-500">No generic drugs found</td>
                                </tr>
                            ) : (
                                sortedMaterials.map((material) => {
                                    const stockStatus = getStockStatus(material);
                                    return (
                                        <tr key={material.id} className="hover:bg-slate-800/50">
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-slate-100">{material.name}</div>
                                                <div className="text-xs text-slate-500">{material.productCode}</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-400">{material.sku || '-'}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-400">{material.supplierName || '-'}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-100">₹{material.costPrice || 0}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-400">{material.currentStock} {material.unitOfMeasure}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">
