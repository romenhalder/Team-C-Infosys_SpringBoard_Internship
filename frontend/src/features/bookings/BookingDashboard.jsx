    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-[#059669] to-[#0d9488] bg-clip-text text-transparent">
                        🎂 Advance Booking
                    </h1>
                ...
                <button onClick={() => { setForm(getEmptyForm()); setEditBooking(null); setShowNewBooking(true); }}
                    className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-[#059669] to-[#0d9488] text-white rounded-xl font-semibold hover:shadow-lg transition-all">
            ...
            <div className="flex gap-2">
                {[{ id: 'dashboard', label: '📋 Orders' }, { id: 'upcoming', label: '🚚 Upcoming' }, { id: 'catalog', label: '🎂 Catalog' }].map(v => (
                    <button key={v.id} onClick={() => setActiveView(v.id)}
                        className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeView === v.id ? 'bg-[#059669] text-white shadow-md' : 'bg-white text-gray-600 border hover:bg-gray-50'
                            }`}>{v.label}</button>
                ))}
            </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                        <input type="text" placeholder="Search orders..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                            className="px-3 py-2 bg-white border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#059669]/20 w-60" />
                        {['ALL', 'PENDING', 'CONFIRMED', 'IN_PROGRESS', 'READY', 'DELIVERED', 'CANCELLED'].map(s => (
                            <button key={s} onClick={() => setStatusFilter(s)}
                                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${statusFilter === s ? 'bg-[#059669] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                    ...
                    {loading ? (
                        <div className="flex justify-center py-12">
                            <div className="animate-spin rounded-full h-10 w-10 border-3 border-[#059669] border-t-transparent"></div>
                        </div>
                                    ...
                                    <div key={booking.id} className="bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all overflow-hidden">
                                        ...
                                        <div className="p-5">
                                            ...
                                            <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                                                <div>
                                                    <span className="text-lg font-bold text-[#059669]">{formatCurrency(booking.estimatedPrice)}</span>
                                                ...
                                                <div className="flex gap-1">
                                                    <button onClick={() => handleEdit(booking)} className="p-1.5 text-gray-400 hover:text-[#059669] hover:bg-[#ecfdf5] rounded-lg">✏️</button>
                            ...
                            {upcoming.map(b => (
                                <div key={b.id} className="bg-white rounded-xl p-4 border shadow-sm flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                        <p className="font-bold text-[#059669]">{b.deliveryDate}</p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {catalog.map(item => (
                                <div key={item.id} className="bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all overflow-hidden group">
                                    <div className="h-40 bg-gradient-to-br from-[#ecfdf5] to-[#FAEBD7] flex items-center justify-center">
                                        {item.imageUrl ? (
                                    ...
                                    <div className="p-4">
                                        <div className="flex items-start justify-between">
                                            <div>
                                                ...
                                                <span className="text-[10px] bg-[#ecfdf5] text-[#059669] px-2 py-0.5 rounded-full font-semibold">{item.category}</span>
                                            </div>
                                            <span className="text-lg font-bold text-[#059669]">{formatCurrency(item.basePrice)}</span>
                                        </div>
                                        ...
                                        <button onClick={() => { selectCatalogItem(item); setShowNewBooking(true); }}
                                            className="w-full mt-3 py-2 bg-gradient-to-r from-[#059669] to-[#0d9488] text-white rounded-lg text-sm font-semibold opacity-0 group-hover:opacity-100 transition-all">
                                            Book Now
                                        </button>
                                    </div>
                                </div>
            ...
            {showNewBooking && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="bg-gradient-to-r from-[#059669] to-[#0d9488] p-5 rounded-t-2xl">
                            <div className="flex justify-between items-center">
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-5">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <input type="text" placeholder="Customer Name *" value={form.customerName} onChange={e => setForm({ ...form, customerName: e.target.value })} required
                                        className="px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#059669]/20" />
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <textarea placeholder="Cake Description" value={form.cakeDescription} onChange={e => setForm({ ...form, cakeDescription: e.target.value })} rows="2"
                                        className="sm:col-span-2 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#059669]/20" />
                            ...
                            <div>
                                        ...
                                        <div className="flex gap-2 mt-1">
                                            {[30, 50, 60, 100].map(p => (
                                                <button key={p} type="button" onClick={() => setForm({ ...form, depositPercentage: String(p) })}
                                                    className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${form.depositPercentage === String(p) ? 'bg-[#059669] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                                        }`}>{p}%</button>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                {form.estimatedPrice && form.depositPercentage && (
                                    <div className="mt-3 p-3 bg-[#ecfdf5] rounded-xl text-sm">
                                        ...
                                        <div className="flex justify-between mt-1"><span className="text-gray-500">Remaining</span>
                                            <span className="font-bold text-[#059669]">{formatCurrency((parseFloat(form.estimatedPrice) * (100 - parseInt(form.depositPercentage)) / 100))}</span></div>
                                    </div>
                                )}
                            </div>

                            <textarea placeholder="Additional notes..." value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} rows="2"
                                className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#059669]/20" />
                            ...
                            <div className="flex gap-3">
                                ...
                                <button type="submit" disabled={loading}
                                    className="flex-1 py-2.5 bg-gradient-to-r from-[#059669] to-[#0d9488] text-white rounded-xl font-bold hover:shadow-lg disabled:opacity-50">
