    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-[#059669] to-[#0d9488] bg-clip-text text-transparent">Cake Catalog</h1>
            ...
            <div className="flex gap-2 flex-wrap">
                <button onClick={() => setFilterCat('ALL')}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold ${filterCat === 'ALL' ? 'bg-[#059669] text-white' : 'bg-gray-100 text-gray-600'}`}>
                    All ({catalog.length})
                </button>
                {CATEGORIES.map(cat => {
                    ...
                    return (
                        <button key={cat} onClick={() => setFilterCat(cat)}
                            className={`px-3 py-1.5 rounded-full text-xs font-semibold ${filterCat === cat ? 'bg-[#059669] text-white' : 'bg-gray-100 text-gray-600'}`}>
            ...
            {loading && catalog.length === 0 ? (
                <div className="flex justify-center py-12"><div className="animate-spin rounded-full h-10 w-10 border-3 border-[#059669] border-t-transparent"></div></div>
            ) : filtered.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {filtered.map(item => (
                        <div key={item.id} className="bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all overflow-hidden group">
                            <div className="h-36 bg-gradient-to-br from-[#ecfdf5] to-[#FAEBD7] relative overflow-hidden">
                                {getImageSrc(item) ? (
                                ...
                                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleEdit(item)} className="p-1.5 bg-white/90 rounded-lg shadow-sm hover:bg-white"><PencilIcon className="h-4 w-4 text-[#059669]" /></button>
                            ...
                            <div className="p-4">
                                <div className="flex items-start justify-between">
                                    ...
                                    <span className="text-sm font-bold text-[#059669]">{formatCurrency(item.basePrice)}</span>
                                </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                        <div className="bg-gradient-to-r from-[#059669] to-[#0d9488] p-5 rounded-t-2xl flex justify-between items-center">
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <div className="mt-1.5">
                                    {imagePreview ? (
                                        <div className="relative rounded-xl overflow-hidden border-2 border-dashed border-[#059669]/30 bg-[#ecfdf5]">
                                            ...
                                            <div className="absolute inset-0 bg-black/0 hover:bg-black/40 transition-all flex items-center justify-center group">
                                                <button type="button" onClick={() => fileInputRef.current?.click()}
                                                    className="opacity-0 group-hover:opacity-100 px-4 py-2 bg-white rounded-lg text-sm font-semibold text-[#059669] shadow-lg transition-all">
                                                    📷 Change Image
                                                </button>
                                            </div>
                                        ...
                                        <button type="button" onClick={() => fileInputRef.current?.click()}
                                            className="w-full h-36 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center gap-2 hover:border-[#059669] hover:bg-[#ecfdf5] transition-all cursor-pointer group">
                                            <div className="p-3 bg-gray-100 rounded-full group-hover:bg-[#ecfdf5] transition-all">
                                                <PhotoIcon className="h-8 w-8 text-gray-400 group-hover:text-[#059669]" />
                                            </div>
                                            <div className="text-center">
                                                <p className="text-sm font-semibold text-gray-500 group-hover:text-[#059669]">Click to upload image</p>
                                ...
                                <div className="grid grid-cols-3 gap-2 mt-1">
                                    {CATEGORIES.map(cat => (
                                        <button key={cat} type="button" onClick={() => setForm({ ...form, category: cat })}
                                            className={`py-2 rounded-lg text-xs font-semibold transition-all ${form.category === cat ? 'bg-[#059669] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                                            {CAT_ICONS[cat]} {cat}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <button type="submit" disabled={loading || uploading}
                                    className="flex-1 py-2.5 bg-gradient-to-r from-[#059669] to-[#0d9488] text-white rounded-xl font-bold hover:shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
                                    {(loading || uploading) ? (
